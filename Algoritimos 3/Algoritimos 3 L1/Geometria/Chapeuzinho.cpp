...
int main()
{
	...
	cin >> n;//qtd de pontos a serem inseridos
	
	for(int i=0; i<n; i++)//inserindo n pontos
		cin >> A[i].x >> A[i].y;
	
	fecho = Graham_scan(A,n);//var aux para receber a pilha
	
	ponto topo_list;//lista aux para dados do topo 
	ponto ultimo_list;//lista aux para dados da base 
	
	//calculo da metragem
	primeiro_list = fecho.back();
	while(!fecho.empty())
	{
		ultimo_list = fecho.back();
		fecho.pop_back();
		
		if(!fecho.empty())
			p += distancia(ultimo_list,fecho.back());
		
		else
			p += distancia(ultimo_list,primeiro_list);	  		
	}
	
	//saida do resultado
	cout << fixed << setprecision(2) << p << " m" << endl;
