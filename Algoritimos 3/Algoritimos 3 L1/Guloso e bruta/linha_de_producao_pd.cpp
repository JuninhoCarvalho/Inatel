...
int main()
{
	...
	int f_; // menor tempo total
	int l_; // linha cuja ultima estação é usada como o caminho mais rápido através de toda a fábrica
	...
	list <int> pilha; // pilha para salvar o caminho
	int linha, maq;
	linha = l_;
	maq = n;
	...
	while(l[linha][maq] != -1)
	{
		pilha.push_back(l[linha][maq]);
		linha = l[linha][maq];
		maq--;
	}
	
	// desempilha mostrando o caminho
	while(!pilha.empty())
	{
		cout << pilha.back() << " ";
		pilha.pop_back();
